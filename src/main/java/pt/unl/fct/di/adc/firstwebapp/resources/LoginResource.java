...
public class LoginResource {
    ...
    public Response doRegistration(LoginData data) {
        LOG.fine("Attempt to register user: " + data.user_name);
        ...
        try {
            ...
            // REMINDER: the info in data will depend on which constructor is used when creating the logindata object.
            // if there is no data on a field, logindata will default to its default values

            Entity.Builder pre = Entity.newBuilder(userKey)
                    .set("user_name", data.user_name)
                    .set("user_pwd", data.user_pwd)
                    .set("user_name_full", data.user_name_full)
                    .set("user_email", data.user_email)
                    .set("user_creation_time", Timestamp.now())
                    .set("user_lastmodified_time", Timestamp.now());

            try{ // OPTIONAL FIELDS, IF FAILS, CREATE INSTANCE OF LOGINDATA AS MINIMAL CONSTRUCTOR
                ...
                // IF IT FAILS, DEFAULT TO DEFAULT VALUES OF MINIMAL CONSTRUCTOR

                LOG.warning("USER ROLE NOT DEFINED, SETTING TO 0");
                data = new LoginData(data.user_email, data.user_name, data.user_pwd, data.user_name_full);
                pre.set("user_role", data.user_role)
                        .set("isPrivate", data.isPrivate)
                        .set("phoneNumber", data.phoneNumber)
                        .set("cellPhoneNumber", data.cellPhoneNumber)
    ...
    public Response doLogin(LoginData data) {
        ...
        if( user != null ) {
            ...
            Long user_role = user.getLong("user_role");
            String hashedPWD = DigestUtils.sha512Hex(user.getString("user_pwd"));

            if(hashedPWD.equals(DigestUtils.sha512Hex(data.user_pwd))){
                AuthToken token = new AuthToken(user_name, user_role);
                LOG.info("USER " + user_name + " LOGIN SUCCESSFUL");
                Transaction txn = datastore.newTransaction();
            ...
            while(results.hasNext()){
                ...
                if(isAuthorNormalUser &&
                    ...
                    currentEntry = Entity.newBuilder(currentEntry.getKey())
                            ...
                            .set("user_name_full", currentEntry.getString("user_name_full"))
    ...
    public Response updateUser(LoginData data,
        ...
        if(authorToken != null){
            ...
            if( toBeChanged == null) {
                return Response.status(Response.Status.BAD_REQUEST)
                        .entity("User does not exist and therefor cannot be updated.").build();
            }

            if( author.getLong("user_role") > toBeChanged.getLong("user_role") ){
                //this.users.remove(nameToBeRemoved);
        ...
        return Response.status(Response.Status.BAD_REQUEST).entity("Invalid token, update user failed.").build();
    }
        ...
        if(authorToken != null){
            ...
            if( toBeChanged != null &&
                ...
                Entity updatedUser = Entity.newBuilder(keyToBeChanged)
                        .set("user_name", toBeChanged.getString("user_name")) // DO NOT ALLOW TO CHANGE USERNAME
                        .set("user_name_full", toBeChanged.getString("user_name_full"))
            ...
            if( author != null ){
                ...
                data.new_pwd.equals(data.new_pwd_conf) ) {
                    ...
                    updatedUser = Entity.newBuilder(author.getKey())
                            ...
                            .set("user_name_full", author.getString("user_name_full"))
